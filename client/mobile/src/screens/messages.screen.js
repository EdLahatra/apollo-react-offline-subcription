    ...
    return {
      headerTitle: (
        <TouchableOpacity
          ...
          onPress={goToGroupDetails} // eslint-disable-line
        >
...
const groupQuery = graphql(GROUP_QUERY, {
  ...
  props: ({
    data: {
      fetchMore, loading, group, refetch, subscribeToMore,
    },
  }) => ({
    loading,
    group,
      ...
      mutate({
        ...
        update: (store, { data: { createMessage } }) => {
          ...
          if (!updatedGroup.messages.edges.length ||
            moment(updatedGroup.messages.edges[0].node.createdAt).isBefore(moment(message.createdAt))) { // eslint-disable-line
